...
segment::segment(vec2_t left_r, vec2_t right_r, vec2_t tl)
{
	this->left = left_r;
	this->right = right_r;

	vec2_t dist = right_r - left_r;
	vec2_t n_(dist.y, -dist.x);
	n_ = glm::normalize(n_);

	
	vec2_t ref = tl - left_r;
	if ((ref.x * n_.x + ref.y * n_.y) < 0) {
		
		this->n = -n_;
		this->l = line(right_r, left_r);
		this->left = right_r;
		this->right = left_r;
	}
	else {
		this->n = n_;
		this->l = line(left_r, right_r);
	}
}
