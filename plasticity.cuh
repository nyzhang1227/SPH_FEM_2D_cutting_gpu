...
__device__ double sigma_yield_saelzer(joco_constants jc, double eps_pl, double eps_pl_dot, double t) {
	double theta = (t - jc.Tref)/(jc.Tmelt - jc.Tref);
...
if (eps_pl > 2){
	...
	Term_A = jc.A + jc.B * pow(eps_pl, jc.n);
}
...
__device__ double sigma_yield_tanh_sima(joco_constants jc, double eps_pl, double eps_dot_pl, double t) {
	...
	double tanh_S =	pow(t/jc.Tmelt, jc.tanh_b);
	//double Term_D = tanh_D + (1.0 - tanh_D) * tanh(1.0/(pow(eps_pl + tanh_S, jc.tanh_c)));
	double Term_D = tanh_D + (1.0 - tanh_D) * pow(tanh(1.0/(pow(eps_pl + tanh_S, jc.tanh_c))), 0.05);
