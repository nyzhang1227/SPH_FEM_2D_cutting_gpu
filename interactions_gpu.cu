...
#ifdef USE_FEM_TOOL
#ifdef SURFACE
texture<float2_tex_t,  1, cudaReadModeElementType> fc_tex;
#endif
#endif
...
__global__ void do_interactions_monaghan(const float_t *__restrict__ blanked, float4_t *__restrict__ v_der, float4_t *__restrict__ S_der,
...
#ifdef SURFACE
	mat2x2_t B(0.);
	float2_t fc_i = texfetch2(fc_tex,pidx);
#endif
	...
	pos_t[pidx] = xi_t;
	vel_t[pidx] = vi_t;
	//if(isnan(vi_t.x)) printf("nan vi_t\n");
...
#ifdef SURFACE
    float_t fc_norm_sqr = fc_i.x * fc_i.x + fc_i.y * fc_i.y;
	if (fc_norm_sqr > 0.) {
	float_t t = B[0][0]+B[1][1];
	float_t det = glm::determinant(B);
	float_t lambda = 0.;

	if (t*t/4-det < 1e-6) {
		lambda = fmin(B[0][0], B[1][1]);
	} else {

		float_t L1 = t/2 + sqrt(t*t/4-det);
		float_t L2 = t/2 - sqrt(t*t/4-det);

		lambda = fmin(L1, L2);
	}
#ifdef Thermal_Conduction_Brookshaw
	if (lambda < 0.875 ) {
		T_t[pidx] +=  tc.hc * (tc.T0 - Ti);
	} 
#endif
	}
#endif
...
#ifdef USE_FEM_TOOL
#ifdef SURFACE
	cudaBindTexture(0, fc_tex, p->fc,   sizeof(float_t)*N*2);
#endif
#endif
